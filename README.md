...
Apple Silicon note: `bitnamilegacy/mongodb:8.0` currently publishes a `linux/amd64` image. The compose file sets `platform: ${MONGO_PLATFORM:-linux/amd64}` for MongoDB so it can run on an M1/M2/M3 Mac through Docker Desktop's amd64 emulation while still running natively on amd64 Linux hosts like Omarchy. The Mongo service runs as root inside the local-dev container so Docker Desktop named volumes can create `/bitnami/mongodb/data/db` without permission errors. If you need to override the platform, set `MONGO_PLATFORM` before running compose.
